        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (userId == null)
                    {
                        return NotFound("Không có id cần tìm");
                    }

                    // ✅ Tạo bản ghi KhachHang với UserId từ tài khoản mới
                    var khachHang = new KhachHang
                    {
                        TenKhachHang = Input.Fullname, // Hoặc có thể lấy từ Input khác nếu cần
                        SoDienThoai = Input.Phone,
                        Email = Input.Email,
                        UserId = userId // Gán ID của user mới vào
                    };

                    await _adminRepository.AddAsync(khachHang); // Thêm vào DbSet                
